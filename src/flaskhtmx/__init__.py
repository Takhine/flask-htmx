...
def create_app():
    app = Flask(__name__)
    jinja_partials.register_extensions(app) # Allows to call the partials in the templates
        ...
        for url, feed_ in feeds.items():
            ...
            for entry in parsed_feed.entries:
                if entry.link not in feed_["entries"]:
                    feed_["entries"][entry.link] = {**entry, "read": False} # destructuring the entry and adding the 'read' key
    ...
    def render_feed_entries(feed_url: str):
        ...
        return render_template("partials/entry_page.html", 
            ...
            href=feed["href"],
            page=page,
            max_page=len(feed["entries"])//5 ## max page number
        )
